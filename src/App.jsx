...
export default function App() {
  ...
  // Medimos la altura del header para fijarlo y compensar el contenido
  const headerRef = useRef(null);
  const [headerH, setHeaderH] = useState(0);
  useEffect(() => {
    const updateHeaderH = () => setHeaderH(headerRef.current?.offsetHeight || 0);
    updateHeaderH();
    window.addEventListener('resize', updateHeaderH);
    return () => window.removeEventListener('resize', updateHeaderH);
  }, []);
  // Exponemos la altura como variable CSS para el scroll-padding-top global
  useEffect(() => {
    try { document.documentElement.style.setProperty('--cc-header-h', `${headerH}px`); } catch {}
  }, [headerH]);
  ...
  {/* Compensamos el contenido para que no quede debajo del header fijo */}
  <style>{`html { scroll-padding-top: var(--cc-header-h, 0px); }`}</style>
